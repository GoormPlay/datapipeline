...
$SPARK_HOME/sbin/start-thriftserver.sh \
  ...
  --master local \
  --packages org.apache.iceberg:iceberg-spark-runtime-3.4_2.12:1.3.1,org.apache.hadoop:hadoop-aws:3.3.2,com.amazonaws:aws-java-sdk-bundle:1.12.262 \
  --jars $SPARK_HOME/jars/iceberg-jars/*.jar \
  --conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions \
  --conf spark.sql.catalog.userlogs_catalog=org.apache.iceberg.spark.SparkCatalog \
  --conf spark.sql.catalog.userlogs_catalog.type=hadoop \
  ...
  --conf spark.hadoop.fs.s3a.access.key=qpJ3MNpmCtpMQw26BURO \
  --conf spark.hadoop.fs.s3a.secret.key=xhrkNGpcVVozn8sAgI7xPsoTfqUxZJgOTwko4DRd \
  --conf spark.hadoop.fs.s3a.region=ap-northeast-2 \
  --conf spark.hadoop.fs.s3a.path.style.access=true \
  --conf spark.hadoop.fs.s3a.connection.ssl.enabled=false \
  --conf spark.hadoop.fs.s3a.impl=org.apache.hadoop.fs.s3a.S3AFileSystem \
  --conf spark.hadoop.fs.s3a.impl.disable.cache=true \
  --conf spark.hadoop.fs.s3a.debug.detailed.exceptions=true
